<?php
class Stemmer extends CCTemplate
{
    ...
    public function __construct()
    {
        ...
        $this->exceptions = new ArrayIterator($this->GetPluralReplacements());
        $this->exceptions2 = new ArrayIterator($this->GetPluralExceptions());
	...
	public function GetPluralReplacements() {
		$repeat_rows = $this->GetDB->query('SELECT word FROM ' . $this->config['table_prefix'] .'stem_exceptions ORDER BY word ASC');
		while ($rows = $repeat_rows->fetch_array(MYSQLI_ASSOC)) {
			$repeat_rows_refresh[] = $rows;
		}
		foreach ($repeat_rows_refresh as $row) {
			$words[$row['word']] = $row['word'];
		}
		return $words;
	}
	public function GetPluralExceptions() {
		$words = array();
		$repeat_rows = $this->GetDB->query('SELECT * FROM ' . $this->config['table_prefix'] .'stem_replace ORDER BY word ASC');
		while ($rows = $repeat_rows->fetch_array(MYSQLI_ASSOC)) {
			$repeat_rows_refresh[] = $rows;
		}
		foreach ($repeat_rows_refresh as $row) {
...
$Stemmer = new Stemmer();
echo '<pre>';
print_r($Stemmer->GetPluralReplacements());
echo '</pre>';
echo $Stemmer->Stem('available');
*/
?>